                      ...
                      {user.addresses.map((a) => (
                        <button key={a.id} onClick={() => setAddr({ cep: a.cep, street: a.street, number: a.number, complement: a.complement || "", neighborhood: a.neighborhood, city: a.city, state: a.state, name: user.name, phone: user.phone })}
                          className={`w-full text-left p-4 border transition-all duration-300 cursor-pointer ${
                            addr.cep === a.cep && addr.street === a.street ? "border-primary/30 bg-primary/5" : "border-foreground/5 hover:border-foreground/10"
                          }`}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-8">
                    {([
                      ...
                      <button key={m.key} onClick={() => setPaymentMethod(m.key)}
                        className={`flex flex-col items-center gap-2 p-4 border transition-all duration-300 cursor-pointer ${
                          paymentMethod === m.key ? "border-primary/30 bg-primary/5" : "border-foreground/5 hover:border-foreground/10"
                        }`}
                        style={{ borderRadius: "var(--radius-card)" }}
                      >
                        ...
                        <div className="mb-4 space-y-2">
                          ...
                          {user.cards.map((c) => (
                            <button key={c.id} onClick={() => { setCardNumber(`**** **** **** ${c.last4}`); setCardName(c.name); setCardExpiry(c.expiry); }}
                              className={`w-full text-left flex items-center gap-4 p-4 border transition-all duration-300 cursor-pointer ${
                                cardNumber.endsWith(c.last4) ? "border-primary/30 bg-primary/5" : "border-foreground/5 hover:border-foreground/10"
                              }`}
                              style={{ borderRadius: "var(--radius-card)" }}
                            >
